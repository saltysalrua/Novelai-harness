...
void main() {
  ...
  group('StudioViewModel.deleteImageFromHistory Tests', () {
    test(
      'deletes image and selects adjacent image or null when empty',
      () async {
    ...
    test('synchronizes board data image nodes when deleting', () async {
      ...
      vm.board.addImageNodeToBoard(refImg);
      ...
      expect(vm.board.boardData.imageNodes.first.image.id, equals('main-img'));
    });
  });

  group('showImageContextMenu UI Widget Tests', () {
    testWidgets(
      'shows context menu with 从历史记录删除 and clicking it invokes deletion',
      (tester) async {
        final repo = NovelAiRepository();
        final vm = StudioViewModel(repository: repo);
