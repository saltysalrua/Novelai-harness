...
**NovelAI Harness** 是一个面向 NovelAI 的桌面端创作工作台。它结合了 **Pi 风格的 AI Harness 架构** 与 **三栏自适应界面**，将参数设置、图像画板、多角色定位、Danbooru 标签生态与 AI 对话交互整合在一起，提供高效的二次元视觉创作体验。
...
- 🤖 **极简 AI Harness 运行时**：
  ...
  - **内置工具箱**：生图/超分、参数读写、多角色增删改查、Danbooru 语义检索/画师推荐、词组合库管理、画板图像查看等。
- ⚡ **NovelAI 协议适配与点数保护**：
  - 全面适配 NovelAI Diffusion V5/V4.5/V4/V3 及 Furry 系列模型。
  - 全局并发锁保护与 429 速率限制重试机制。
  - 内置 **Anlas 预计消耗计算**，支持 Opus 免费档规则（像素 ≤ 1,048,576 且步数 ≤ 28 且单张）与 V5 体力配额检测。
  - 适配 2026-08 官方新版超分协议（Multipart 表单传输、新超分模型固定倍率、字节级尺寸解析）。
...
- 📝 **提示词编辑与语法高亮**：
  - 基于 AST 解析提示词结构，支持 `{}`/`[]` 权重即时增减、SD ⇄ NAI 语法双向转换与标签禁用（`#` / 删除线）。
  - 富文本语法高亮控制器，根据 Danbooru 官方分类（Artist/Character/Copyright/General/Meta）着色。
...
本项目基于 **[MIT License](LICENSE)** 开源。在此向所引用的开源项目与开源资源致谢：
...
本项目采用 [MIT License](LICENSE) 许可证开源。请在使用前仔细阅读上述 [第三方提醒与免责声明](#️-第三方提醒与免责声明-notices--disclaimers)。
